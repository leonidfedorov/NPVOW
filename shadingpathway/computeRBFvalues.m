...
%
%                Version 1.0,  21 October 2015 by Leonid Fedorov.
%
...
if ( m ~= k)
    ...
    % we can compute column diffs of each pair of column vectors a single call
    diffs = bsxfun(@minus, centers, inputs);
    
    % translates is a matrix of column vectors, that are diffs
    %translates = reshape(diffs, [m l*n]);
    
    % compute squared l2 norm of each column of translates
